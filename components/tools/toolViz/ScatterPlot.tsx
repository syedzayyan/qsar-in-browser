...
export default function Scatterplot({
  ...
  useEffect(() => {
    ...
    setOuterSize({
      width: Math.max(300, Math.floor(init.width)),
      height: Math.max(300, Math.floor(init.width * 0.65))
    });
  ...
  const colorDomain = colorProperty.length
    ? [d3.min(colorProperty)!, d3.max(colorProperty)!]
    : [0, 1];
  ...
  const createSelectionHandlers = useCallback(
    (
      ...
      const handleMouseMove = (event: MouseEvent) => {
        ...
        selectionRect.attr('x', x0).attr('y', y0).attr('width', wSel).attr('height', hSel);
      };
