  ...
  export class NSGA2 {
    ...
    protected sort(individuals: Individual[]): Individual[][] {
      ...
      for (let i = 0; i < l; i++) {
        ...
        for (let j = 0; j < l; j++) {
          ...
          if (individuals[i].dominate(individuals[j])) {
            individuals[i].individualsDominated.push(individuals[j]);
      ...
      while (fronts[rank].length > 0) {
        ...
        for (let k = 0; k < fronts[rank].length; k++) {
          for (
            let j = 0;
            j < fronts[rank][k].individualsDominated.length;
            j++
          ) {
            fronts[rank][k].individualsDominated[j].dominatedCount -= 1;
            if (fronts[rank][k].individualsDominated[j].dominatedCount == 0) {
              fronts[rank][k].individualsDominated[j].paretoRank = rank + 2;
    ...
    protected sortByObjective(individuals: Individual[], objectiveId: number) {
      ...
      for (let i = 0; i < individuals.length; i++) {
        for (let j = i; j > 0; j--) {
          if (
            individuals[j].objectives[objectiveId] -
              individuals[j - 1].objectives[objectiveId] <
            0
          ) {
            tmp = individuals[j];
    ...
    protected mate(parentA: Individual, parentB: Individual): Individual[] {
      ...
      childs[0].chromosome = parentA.chromosome.slice(0, this.chromosomeSize);
      childs[1].chromosome = parentB.chromosome.slice(0, this.chromosomeSize);
      // Crossovers
    ...
    protected getGoodParent(parents: Individual[]): Individual {
      ...
      if (parents[r[0]].paretoRank == parents[r[1]].paretoRank) {
        if (parents[r[0]].crowdingDistance >= parents[r[1]].crowdingDistance) {
          return parents[r[0]];
        }
        if (parents[r[0]].crowdingDistance < parents[r[1]].crowdingDistance) {
          return parents[r[1]];
    ...
    protected sortByCrowdingDistance(individuals: Individual[]) {
      ...
      for (let i = 0; i < individuals.length; i++) {
        for (let j = i; j > 0; j--) {
          if (
            individuals[j].crowdingDistance -
              individuals[j - 1].crowdingDistance <
            0
          ) {
            tmp = individuals[j];
            individuals[j] = individuals[j - 1];
